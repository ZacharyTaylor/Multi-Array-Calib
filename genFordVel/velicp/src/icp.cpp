...
void Icp::fit (double *T,double *time,const int32_t T_num,Matrix &R,Matrix &t,const double indist) {
  ...
  for (int32_t i=0; i<T_num; i+=sub_step)
    active.push_back(i);
  fitIterate(T,time,T_num,R,t,active);
  ...
  if (indist<=0) {
    ...
    active = getInliers(T,time,T_num,R,t,indist);
  }
  fitIterate(T,time,T_num,R,t,active);
}

void Icp::fitIterate(double *T,double *time,const int32_t T_num,Matrix &R,Matrix &t,const std::vector<int32_t> &active) {
  ...
  for (int32_t iter=0; iter<max_iter; iter++)
    if (fitStep(T,time,T_num,R,t,active)<min_delta)
      break;
}
