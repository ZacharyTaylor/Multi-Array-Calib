...
double IcpPointToPlane::fitStep (double *T,double *time,const int32_t T_num,Matrix &R,Matrix &t,const std::vector<int32_t> &active) {
  ...
  Matrix p_t(active.size(),dim); // template
    ...
    for (int32_t i=0; i<active.size(); i++) {
      ...
      query[0] = (r00*T[idx*3+0] + r01*T[idx*3+1] + r02*T[idx*3+2] + t0 -T[idx*3+0])*(time[idx]+1) + T[idx*3+0];
      query[1] = (r10*T[idx*3+0] + r11*T[idx*3+1] + r12*T[idx*3+2] + t1 -T[idx*3+1])*(time[idx]+1) + T[idx*3+1];
      query[2] = (r20*T[idx*3+0] + r21*T[idx*3+1] + r22*T[idx*3+2] + t2 -T[idx*3+2])*(time[idx]+1) + T[idx*3+2];
...
std::vector<int32_t> IcpPointToPlane::getInliers (double *T,double *time,const int32_t T_num,const Matrix &R,const Matrix &t,const double indist) {
  ...
  kdtree::KDTreeResultVector neighbor;
    ...
    for (int32_t i=0; i<T_num; i++) {
      ...
      // transform point according to R|t
      double sx = (r00*T[i*3+0] + r01*T[i*3+1] + r02*T[i*3+2] + t0 -T[i*3+0])*(time[i]+1) + T[i*3+0]; query[0] = sx;
      double sy = (r10*T[i*3+0] + r11*T[i*3+1] + r12*T[i*3+2] + t1 -T[i*3+1])*(time[i]+1) + T[i*3+1]; query[1] = sy;
      double sz = (r20*T[i*3+0] + r21*T[i*3+1] + r22*T[i*3+2] + t2 -T[i*3+2])*(time[i]+1) + T[i*3+2]; query[2] = sz;
