...
double IcpPointToPoint::fitStep (double *T,double *time,const int32_t T_num,Matrix &R,Matrix &t,const std::vector<int32_t> &active) {
...
std::vector<int32_t> IcpPointToPoint::getInliers (double *T,double *time,const int32_t T_num,const Matrix &R,const Matrix &t,const double indist) {
  ...
  // dimensionality 3
    ...
    for (int32_t i=0; i<T_num; i++) {
      ...
      // transform point according to R|t
      query[0] = (r00*T[i*3+0] + r01*T[i*3+1] + r02*T[i*3+2] + t0 -T[i*3+0])*(time[i]+1) + T[i*3+0];
      query[1] = (r10*T[i*3+0] + r11*T[i*3+1] + r12*T[i*3+2] + t1 -T[i*3+1])*(time[i]+1) + T[i*3+1];
      query[2] = (r20*T[i*3+0] + r21*T[i*3+1] + r22*T[i*3+2] + t2 -T[i*3+2])*(time[i]+1) + T[i*3+2];
