function [ velData ] = genFordVel( fordPath, plotVel, range )
...
vel = [vel.vertex.x, vel.vertex.y, vel.vertex.z, vel.vertex.timeOffset, vel.vertex.valid];
vel = vel(vel(:,5)>0,1:4);
vel(:,4) = vel(:,4)/(2*max(abs(vel(:,4))));
...
for frame = 2:size(velData.files,1)
    ...
    [velData.T_Skm1_Sk(frame,:), velData.T_Cov_Skm1_Sk(frame,:)] = getTvel(velOld, vel, velData.T_Skm1_Sk(frame-1,:));
