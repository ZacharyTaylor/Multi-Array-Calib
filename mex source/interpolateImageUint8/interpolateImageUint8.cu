...
__global__ void NearInterpolateKernel(const uint8_t* const imageIn,
										float* const out,
										const size_t height,
										const size_t width,
										const float* const x,
                 ...
                 int nrhs, mxArray const *prhs[])
{
    ...
    uint8_t* imagePtr = (uint8_t*)(mxGPUGetDataReadOnly(image));
	...
	for(size_t i = 0; i < imageDepth; i++){
		uint8_t* imageLayerPtr = &(imagePtr[imageHeight*imageWidth*i]);
		float* outLayerPtr =  &(outPtr[numPoints*i]);
		NearInterpolateKernel<<<gridSize(numPoints), BLOCK_SIZE>>>(imageLayerPtr, outLayerPtr, imageHeight, imageWidth, xPtr, yPtr, numPoints);
		CudaCheckError();
	}
