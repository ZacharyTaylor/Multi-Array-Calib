                 ...
                 int nrhs, mxArray const *prhs[])
{
    ...
    mxGPUArray const * distVec = mxGPUCreateFromMxArray(prhs[2]);
    mxGPUArray const * pointsMat = mxGPUCreateFromMxArray(prhs[3]);
    ...
    float* distPtr = (float*)(mxGPUGetDataReadOnly(distVec));
    ...
    //run and get ouputs
	CameraTransformKernel<<<gridSize(numPoints), BLOCK_SIZE>>>(tformPtr, camPtr, distPtr, imWidth, imHeight, xInPtr, yInPtr, zInPtr, numPoints, xOutPtr, yOutPtr,validPtr);
	CudaCheckError();
	
    //destroy reference structures
    mxGPUDestroyGPUArray(tformMat);
    mxGPUDestroyGPUArray(camMat);
    mxGPUDestroyGPUArray(distVec);
