...
%path to data
shrimpPath = '/home/z/Documents/Datasets/Shrimp/around-shed';
...
velData = matchTforms(velData, cam1Data,range, true);
...
% navData = matchTforms(navData, camData{1},range, false);
...
%% process cam
load('shrimpCam1Data.mat');
sensorData{tformIdx} = cam1Data;
tform{tformIdx} = cam1Data.T_Skm1_Sk(range,:);
tformVar{tformIdx} = cam1Data.T_Cov_Skm1_Sk(:,:,range);
tformIdx = tformIdx + 1;

load('shrimpCam2Data.mat');
sensorData{tformIdx} = cam2Data;
tform{tformIdx} = cam2Data.T_Skm1_Sk(range,:);
tformVar{tformIdx} = cam2Data.T_Cov_Skm1_Sk(:,:,range);
tformIdx = tformIdx + 1;
...
% tformIdx = tformIdx + 1;
% 
% sensorData{tformIdx} = camData{5};
% tform{tformIdx} = camData{5}.T_Skm1_Sk(range,:);
% tformVar{tformIdx} = camData{5}.T_Cov_Skm1_Sk(range,:);
% tformIdx = tformIdx + 1;
...
for w = 100%1:100
        w
    for l = 500%10:10:1000
        l

        %get random index to use for scans
        idx = randi(900-l+10)+5;
        idx = (idx:idx+l)';
