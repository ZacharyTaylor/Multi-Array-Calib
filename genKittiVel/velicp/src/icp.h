...
public:
  ...
  //         time .... fraction of time for scan
  ...
  void fit(double *T, double *time, const int32_t T_num,Matrix &R,Matrix &t,const double indist);
  
private:
  
  // iterative fitting
  void fitIterate(double *T, double *time, const int32_t T_num,Matrix &R,Matrix &t,const std::vector<int32_t> &active);
  
  // inherited classes need to overwrite these functions
  virtual double               fitStep(double *T,double *time,const int32_t T_num,Matrix &R,Matrix &t,const std::vector<int32_t> &active) = 0;
  virtual std::vector<int32_t> getInliers(double *T,double *time,const int32_t T_num,const Matrix &R,const Matrix &t,const double indist) = 0;
