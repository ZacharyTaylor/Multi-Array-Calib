...
for i = 1:length(camData)
    ...
    eq1 = ones(3,size(camData{i}.T_Skm1_Sk,1),samples);
    eq2 = ones(3,size(camData{i}.T_Skm1_Sk,1),samples);
    eq3 = ones(3,size(camData{i}.T_Skm1_Sk,1),samples);
    eq4 = ones(3,size(camData{i}.T_Skm1_Sk,1),samples);
    ...
    for j = 3:size(camData{i}.T_Skm1_Sk,1)
        for k = 1:samples
            ...
            eq1(:,j-1,k) = (rb23-eye(3))\(R*ta23);
            eq2(:,j-1,k) = (rb12-eye(3))\(R*ta12);
            eq3(:,j-1,k) = (rb23-eye(3))\tb23;
            eq4(:,j-1,k) = (rb12-eye(3))\tb12;

            s(j-1,k) = (eq3(:,j-1,k)-eq4(:,j-1,k))\(eq1(:,j-1,k)-eq2(:,j-1,k));
        end
    end
