...
for b = 2:size(sensorData,1)
    ...
    for j = 1:s
        Rb(3*j-2:3*j,1:3) = RMat(:,:,b)*(eye(3) - V2R(sensorData{b}.T_Skm1_Sk(j,4:6)));
    ...
    for j = 1:s
        Ra(3*j-2:3*j,1:3) = V2R(sensorData{1}.T_Skm1_Sk(j,4:6)) - eye(3);
        Ra(3*j-2:3*j,:) = weight(j).* Ra(3*j-2:3*j,:);

        tb(3*j-2:3*j) = RMat(:,:,b)*sensorData{b}.T_Skm1_Sk(j,1:3)' - sensorData{1}.T_Skm1_Sk(j,1:3)';
